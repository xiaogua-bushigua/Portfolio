...
const Cloud = React.memo(({ refresh }) => {
	...
	return (
		<Instances castShadow receiveShadow>
			...
			{new Array(num).fill(0).map((_, index) => (
				...
				>
					<Instance position={[-1.85, Math.random() * 0.4 + 3, 0]} scale={1.2} />
					<Instance position={[0, Math.random() * 0.4 + 3, 0]} scale={1.5} />
					<Instance position={[1.85, Math.random() * 0.4 + 3, 0]} scale={0.9} />
				</group>
			))}
		</Instances>
	);
